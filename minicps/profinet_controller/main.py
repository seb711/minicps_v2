...
class PNConnection:
    def __init__(self, iface, mac_address, device_path, state):
    ...
    def initialize_device(self, ip, name, prev_device_name=""):
        # self.device.identify(prev_device_name)
        if not self.device.setIp(ip=ip): 
            print("IP COULD NOT BE SET. CONTROLLER ABORTS ACTION WITH DEVICE ", name)
            return False
        if not self.device.setName(name): 
            print("NAME COULD NOT BE SET. CONTROLLER ABORTS ACTION WITH DEVICE ", name)
            return False

        return True

    def connect_device(self, ip, parameter_list=""):
    ...
    def start_consumer_provider(self):
        self.provider.sendMsgs()
        self.consumer.receiveMsgs()
        return True

    def stop_consumer_provider(self):
...
def main():
    ...
    if not connection.initialize_device(ip=args.ip, name=args.name, prev_device_name=""): 
        print("CONNECTION NOT POSSIVBLE DUE TO CONFIGURATION ERROR")
        return -1
