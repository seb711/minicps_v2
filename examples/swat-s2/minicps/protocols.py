...
class Protocol(object):
    ...
    def _stop(self):
        """Stop all processes of the protocol instance
...
class EnipProtocol(Protocol):
    ...
    def _start_server(cls, address, tags):
        ...
        try:
            ...
            subprocess.Popen([sys.executable + ' -m cpppo.server.enip.io '], shell=False)
            return server
    ...
    def _stop(self): 
        self._stop_server(self._server_subprocess)
...
class PnioProtocolController(Protocol):

    def __init__(self, protocol):

        super(PnioProtocolController, self).__init__(protocol)

        # NOTE: set up logging
        if sys.platform.startswith('linux'):
            self._client_log = 'logs/pnio_client '
        else:
            raise OSError
    ...
    # TODO: parametric PRINT_STDOUT and others
    @classmethod
    def _start_server_cmd(cls, device, db_path, server_name):
        ...
        CMD = 'cd ./minicps/profinet_controller/; sudo python3 main.py '
        # print 'DEBUG: enip _start_server_cmd HTTP: ', HTTP
        PATH = '--path ' + "./gsdml/minicps_device.xml" + ' '
        MAC = '--mac ' + device["mac"] + ' '
        IFACE = '--iface ' + "%s-eth0"%(server_name) + ' '
    ...
    def _stop(self): 
        try:
            print("DEBUG Stop all servers.") 
            for (label, _, _) in self._server['tags']:
                self._stop_server(self._server_subprocess[label])
        except Exception as error:
            print 'ERROR stop all servers: ', error

    def _send(self, what, value, address, **kwargs):
        print("ACTIVE VALUE: ", float(self.dbstate[what[0]]._get(("ACTIVE", 1))))
        if float(self.dbstate[what[0]]._get(("ACTIVE", 1))) > 0: 
            self.dbstate[what[0]]._set((self.used_input_register[what[0]], 1), value)
        else: 
            self._stop_server(self._server_subprocess[what[0]])
...
class PnioProtocolDevice(Protocol):
    ...
    def __init__(self, protocol):
        ...
        super(PnioProtocolDevice, self).__init__(protocol)
        ...
        self._server_subprocess = PnioProtocolDevice._start_server(
                    address=self._server['address'],
                    tags=self._server['tags'], 
                    device_name=self._server['name'],
                    db_path=self._db_path)

    @classmethod
    def _start_server(cls, address, tags, db_path, device_name):
        """Start a cpppo enip server.
    ...
    # TODO: parametric PRINT_STDOUT and others
    @classmethod
    def _start_server_cmd(cls,  db_path, device_name):

        CMD = 'cd ./minicps/profinet_device/build/; sudo ./pn_dev '
        # print 'DEBUG: enip _start_server_cmd HTTP: ', HTTP
